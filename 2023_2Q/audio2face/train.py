...
if __name__ == '__main__':
    ...
    # Path
    project_dir = g.projDir
    output_path = os.path.join(project_dir, g.Dirs.output)
    output_path_under = os.path.join(output_path, g.GetCurrentTime())

    checkpoint_save_path = g.GetPath(g.Dirs.output, g.GetCurrentTime(), g.Dirs.ckpt, g.Dirs.Audio2Face)
    model_save_path = os.path.join(output_path, g.Dirs.model, g.Dirs.Audio2Face)

    # Create output folder
    # if not os.path.exists(output_path):
    #     os.mkdir(output_path)
    g.CreateFolder(g.Dirs.output, g.GetCurrentTime(), g.Dirs.ckpt, g.Dirs.Audio2Face)
    g.CreateFolder(g.Dirs.output, g.GetCurrentTime(), g.Dirs.model, g.Dirs.Audio2Face)
    ...
    # Load data
    x_train = np.load(g.GetPath(g.Dirs.dataSet, 'train_data.npy'))
    x_val = np.load(g.GetPath(g.Dirs.dataSet, 'val_data.npy'))
    y_train = np.load(g.GetPath(g.Dirs.dataSet, 'train_label_var.npy'))
    y_val = np.load(g.GetPath(g.Dirs.dataSet, 'val_label_var.npy'))
