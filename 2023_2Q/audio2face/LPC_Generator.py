...
#https://github.com/FACEGOOD/FACEGOOD-Audio2Face/tree/main/code/train


projDir = g.projDir
dll = cdll.LoadLibrary(os.path.join(projDir, "liblpc.so"))
#파일 리스트로 만들어 주세요 --
#nameList = ['1_01']#, '1_02', '2_01', '2_02']
nameList = ['001_iPhone']#, '1_02', '2_01', '2_02']

#wav_path = [os.path.join(projDir, 'wav', '1114_' + file + '.wav') for file in nameList] 
#save_path = [os.path.join(projDir, 'lpc', '1114_' + file + '.npy') for file in nameList]  
wav_path = [os.path.join(projDir, 'wav', 'VIVE_JUSTIN_TTS_' + file + '.wav') for file in nameList] 
save_path = [os.path.join(projDir, 'lpc', 'VIVE_JUSTIN_TTS_' + file + '.npy') for file in nameList]
#파일 리스트로 만들어 주세요 --


g.CreateFolder(g.Dirs.lpc)
...
def audioProcess(wav_path):
    ...
    t1_end = process_time()
    print("Elaapsed Time = ", t1_end - t1_start)
    print(inputData_array.shape)

    return inputData_array

    
#https://github.com/FACEGOOD/FACEGOOD-Audio2Face/blob/main/code/train/step1_LPC.py
