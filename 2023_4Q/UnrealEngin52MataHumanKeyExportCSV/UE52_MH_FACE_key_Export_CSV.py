...
csvs = 'csvs'
targetPath = os.path.join(Path.home(), csvs)
if os.path.exists(targetPath) == False:
    os.mkdir(targetPath)
    
import time 
curr_time = int(time.time()) / 86400
curr_time = int(curr_time)

savecsv = str(editor_asset_name +'_'+ str(curr_time) + '.csv')
fullpath = os.path.join(targetPath, savecsv)
...
for i in writeCSV:
    wr.writerow(i) 
f.close()           
print (f'Export Finished : {fullpath}') 
