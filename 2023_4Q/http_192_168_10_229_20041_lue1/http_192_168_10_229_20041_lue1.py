#http_192_168_10_229_20041_lue1.py
import sys
sys.version
...
def server_http_192_168_10_229_20041_lue1(presetNumberInt):      
    create_progressBar_Win('...http.........')
    progressNum=0
    #start        
    KEY_WAV = "wav"
    url = "http://192.168.10.229:20041/lue1"
    headers = {
        "Content-Type": "application/json"
    }
    payload = json.dumps({
      "presetNumber": presetNumberInt,
      "text": " "
    })
    ...
    delete_progressBar_Win()  
    return inData
def sound_wav(inData,number):
    create_progressBar_Win('Create Sound')
    progressNum=0
    decord2byte = base64.b64decode(inData["wav"])
    # 사운드 파일 경로
    filename = f"192_168_10_229_20041_lue1_{number}_{time}.wav"
...
def getJsonInData(inData,stringJsonKey):
    ...
    print ( endframe )
def http_192_168_10_229_20041_lue1(number):
    pm.currentUnit(time='ntsc')  
    deleteARkitKey()
    inData = server_http_192_168_10_229_20041_lue1(number)
    sound_wav(inData,number)
    getJsonInData(inData,"json")
    print (number,len(inData['json']))
    #pm.mel.eval('playButtonForward;')


    filename = f"192_168_10_229_20041_lue1_{number}_{time}.mov"
    full = os.path.join("d:", filename)
...
"""    
for i in range(0,5):
    http_192_168_10_229_20041_lue1(i) 
"""    
http_192_168_10_229_20041_lue1(4)     
